...
describe("Original projector functionality", () => {
  ...
  it("renders the virtual DOM immediately when adding renderFunctions", () => {
    ...
    expect(parentElement.insertBefore.lastCall.args[1]).to.equal(
      siblingElement
    );
    ...
    expect(existingElement.appendChild.lastCall.args[0].tagName).to.equal(
      "SPAN"
    );
  ...
  describe("Event handlers", () => {
    ...
    class ButtonComponent implements MaquetteComponent {
      ...
      constructor(
        buttonText: string,
        buttonClicked: (sender: ButtonComponent) => void
      ) {
      ...
      public render() {
        return h("button", { onclick: this.handleClick, bind: this }, [
          this.text,
        ]);
      }
    ...
    it('invokes the eventHandler with "this" set to the value of the bind property', () => {
      ...
      let buttonElement = parentElement.appendChild.lastCall
    ...
    let allowsForEventHandlersToBeChanged = (
      createProjectorImpl: (arg: any) => Projector
    ) => {
    ...
    it("allows for eventHandlers to be changed on IE11", () => {
      ...
      try {
        ...
        delete require.cache[
          path.normalize(path.join(__dirname, "../src/projector.ts"))
        ];
        let createProjectorImpl =
          require("../src/projector").createAdvancedProjector;
